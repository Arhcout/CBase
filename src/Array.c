...
void *ManualArrayPushBulk(void *arr, void *data, size_t size) {
  ...
  if (state->len > state->maxLen) {
    do
      state->maxLen *= 2;
    while (state->len > state->maxLen);
    state =
        realloc(state, state->maxLen * state->typeSize + sizeof(struct Array));
...
void ArrayRemove(void *arr, size_t index) {
  ...
  void *startCpy =
      (void *)(((uint64_t)arr + index * state->typeSize) + state->typeSize);
  // Get the copy length
  uint64_t lenCpy =
      ((uint64_t)arr + state->len * state->typeSize) - (uint64_t)startCpy;
...
void *CopyArray(void *arr) {
  ...
  struct Array *state = arr - sizeof(struct Array);
  struct Array *new =
      malloc(sizeof(struct Array) + state->typeSize * state->len);
